...
class ExtractTest extends PHPUnit_Framework_TestCase {
	...
	function test_with_just_a_string() {
		...
		$result = $this->extractor->extract_entries('<?php __("baba",ET_DOMAIN); ?>', 'baba.php' );
	...
	function test_find_function_calls_one_arg_literal() {
		$this->assertEquals( array( array( 'name' => '__', 'args' => array( 'baba' ), 'line' => 1 ) ), $this->extractor->find_function_calls( array('__'), '<?php __("baba",ET_DOMAIN); ?>' ) );
	}

	function test_find_function_calls_one_arg_zero() {
		$this->assertEquals( array( array( 'name' => '__', 'args' => array( '0' ), 'line' => 1 ) ), $this->extractor->find_function_calls( array('__'), '<?php __("0", ET_DOMAIN); ?>' ) );
	}
