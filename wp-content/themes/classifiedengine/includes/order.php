<?php
class ET_AdOrder extends ET_Order {
	...
	public function __construct( $order, $ship = array ()){
		if( is_array($order) ) {
			...
			$this->payment_plan = $order['payment_plan'];
	...
	{
		$comments = get_comments('post_id=' . $this->_ID);
		if (!empty($comments)) {
	...
	public function add_product ( $product , $number = 1) {
		...
		$this->_products[$product['ID']]	=	 array (
				...
				'AMT'		=>  $product[CE_ET_PRICE],
				'QTY'		=>	$number,
				'L_DESC'	=>	$product['post_content']
			) ;
		$this->_total_before_discount	+=		number_format($product[CE_ET_PRICE]*$number, 2, '.', '');
