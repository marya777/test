...
abstract  class ET_Payment {
	...
	public static function get_support_payment_gateway () {
		...
		if (class_exists('WooCommerce')) {
			...
			foreach ($available_payment_gateways as $gateway) {
				$wc_support[$gateway->id]['label'] = $gateway->title;
				$wc_support[$gateway->id]['description'] = $gateway->method_description;
				$wc_support[$gateway->id]['active'] = ($gateway->enabled == 'yes') ? 1 : -1;
			}
			$support = array_merge($support, $wc_support);
		}
	...
	public static function get_currency_list () {
		...
		if (class_exists('WooCommerce') && function_exists("get_woocommerce_currencies")) {
			...
			$currency = array();
			foreach ($available_currencies as $code => $alt) {
				$currency[$code]['code'] = $code;
				$currency[$code]['alt'] = $alt;
				$currency[$code]['label'] = $code;
				$currency[$code]['icon'] = get_woocommerce_currency_symbol($code);
				$currency[$code]['align'] = 'left';
			}
		} else {
			$currency = array(
	...
	public static function get_currency (  ) {
		...
		if(class_exists("WooCommerce"))
		{
			$wcCur = get_woocommerce_currency();
			$wcCurrencies = get_woocommerce_currencies();
			$cur = array(
						'code' => $wcCur,
						'alt' => $wcCurrencies[$wcCur],
						'label' => $wcCur,
						'icon' => get_woocommerce_currency_symbol($wcCur),
						'align' => 'left'
					);
		} else {
		...
		if (class_exists('WooCommerce')) {
			...
			foreach ($available_payment_gateways as $gateway) {
				if ($gateway->enabled === 'yes') {
					$wc_support[$gateway->id]['label'] = $gateway->title;
					$wc_support[$gateway->id]['description'] = $gateway->method_description;
					$wc_support[$gateway->id]['active'] = 1;
				}
			}
			$gateways = array_merge($gateways, $wc_support);
		}
	...
	*/
	...
	/**
	* get payment disbale status
	* return bool : true : test, false is real
	*/
	public static function get_payment_disable () {
		$opt	= get_option ('et_payment_disable', false);
		if (is_array($opt) || empty($opt))
			return false;
		else return $opt;
	}
	...
	* return bool : true : test, false is real
	*/
	public static function set_payment_disable ($value) {
		return update_option ('et_payment_disable', $value);
	}
    ...
    */
    ...
    public function successful_request( $posted ) {
        ...
        if ( ! empty( $posted['invoice'] )) {
            $order_pay = new ET_AdOrder($posted['invoice']);
    ...
    function check_ipn_request_is_valid ( $ipn_response )
    {
        ...
        $params = array(
            ...
            'compress'      => false,
            'decompress'    => false,
            'user-agent'	=> 'ClassifinedEngine/'.CE_VERSION
        );
...
function et_enable_gateway ($gateway) {
	...
	if (class_exists("WooCommerce")) {
		$available_payment_gateways = WooCommerce::instance()->payment_gateways()->get_available_payment_gateways();
		$paymentType_lower = strtolower($a);
		if (array_key_exists($paymentType_lower, $available_payment_gateways)) {
			$available = $available_payment_gateways[$paymentType_lower]->is_available();
		}
	}
...
/**
 * update disable payment
 * @param bool
*/
function et_set_payment_disable ($value) {
	ET_Payment::set_payment_disable ($value);
}
/**
 * get payment test mode
 * @return bool
*/
function et_get_payment_disable () {
	return ET_Payment::get_payment_disable ();
}
/**
