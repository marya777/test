...
function et_customizer_init(){
	...
	*/
	...
	$option 			= CE_Options::get_instance();
	$customization 		= $option->get_customization();
	// check if customization is create or not
	if(!is_multisite() || (is_multisite() && get_current_blog_id() == 1) )
		if ( !file_exists( get_template_directory()  . '/css/customization.css' ) ){
		...
		if ( !file_exists( get_template_directory()  . '/css/customization_{$site_id}.css' ) ){
			$option 			= CE_Options::get_instance();
			$customization 		= $option->get_customization();
			$customization['pattern'] = "'" . $customization['pattern'] . "'";
...
function et_get_customization_option(){
	$option = CE_Options::get_instance();
	$style 	= $option->get_customization();
	return $style;
}
...
function et_set_customization_option($value){
	$option = CE_Options::get_instance();
	$style 	= $option->set_customization();
}
...
function et_customization_styles(){
	...
	if( get_theme_mod( 'ce_minify' , 0 ) ) return false;
	...
	if ( file_exists( $file ) ){
		...
		wp_enqueue_style( 'et-customization', $url);
...
function et_customizer_print_styles(){
	if ( current_user_can('manage_options') && !is_admin()){
		...
		wp_register_style( 'et_colorpicker', FRAMEWORK_URL. '/js/lib/css/colorpicker.css', array('custom') );
...
function et_customizer_print_scripts(){
	...
	if ( current_user_can('manage_options')&& !is_admin() ){
		...
		wp_enqueue_script('et-colorpicker', false, array('jquery'), '1.0', true);

		// scrollbar
		wp_register_script( 'et-tinyscrollbar', TEMPLATEURL . '/js/lib/jquery.tinyscrollbar.min.js' );
		wp_enqueue_script('et-tinyscrollbar', false, array('jquery', 'underscore', 'backbone'), '1.0', true );

		// customizer script
		wp_register_script('et_customizer', TEMPLATEURL . '/js/customizer.js', array('jquery','et-colorpicker'), false, true);
		wp_enqueue_script('et_customizer', false, array( 'jquery', 'et-colorpicker'), '1.0', true);
		?>
		<link rel="stylesheet/less" type="txt/less" href="<?php echo TEMPLATEURL . '/css/define.less'?>">
		<?php
		wp_register_script( 'less-js', TEMPLATEURL . '/js/lib/less-1.4.1.min.js', '1.0', true );
...
function et_customizer_save(){
	...
	try {
		...
		$option = CE_Options::get_instance();
		$style 	= $option->set_customization($customization);

		$resp = array(
			'success' 	=> true,
			'code' 		=> 200,
			'msg' 		=> __("Changes are saved successfully.", ET_DOMAIN),
			'data' 		=> $option->get_customization()
		);
	} catch (Exception $e) {
		$resp = array(
			'success' 	=> false,
...
function et_apply_customization($options = array(), $preview = false){
	...
	$less = get_template_directory()  . '/css/customization.less';
	// $less = get_template_directory()  . '/css/custom-et.less';
	// if ( $preview )
	// 	$css = get_template_directory()  . '/css/customization-preview.css';
	// else
	if( is_multisite() ) {
				...
				<div class="section">
					...
					<div class="section-content">
						<ul class="blocks-grid">
							<li class="clr-block scheme-item" data="" style="background: #8E44AD"></li>
							<li class="clr-block scheme-item" data="" style="background: #2980B9"></li>
							<li class="clr-block scheme-item" data="" style="background: #1BA084"></li>
							<li class="clr-block scheme-item" data="" style="background: #904C09"></li>
							<li class="clr-block scheme-item" data="" style="background: #E67E22"></li>
							<li class="clr-block scheme-item" data="" style="background: #16A084"></li>
							<li class="clr-block scheme-item" data="" style="background: #AD0A4B"></li>
				...
				<div class="section">
					...
					<div class="section-content" style="display: none">
						<?php $fonts = apply_filters ('et_customize_fonts_face',array(
							...
							'opensans'		=> array( 'fontface' => 'Open Sans, sans-serif', 'name' => 'Open Sans' ),
...
function et_set_layout($new_layout){
	$option = CE_Options::get_instance();
	$style 	= $option->set_layout($new_layout);
}
