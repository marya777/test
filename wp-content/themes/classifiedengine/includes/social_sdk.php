...
abstract class ET_SocialAuth extends ET_Base{
	...
	public function enqueue_scripts(){
		if ( is_page_template( 'page-social-connect.php' ) ){
			$this->add_script('et-authentication', TEMPLATEURL . '/js/authentication.js', array('jquery', 'underscore', 'backbone','ce'),false,true);
...
class ET_FaceAuth extends ET_SocialAuth{
	...
	const FB_APP_ID 		= 'et_fabbook_id';
	const FB_APP_SECRET 	= 'et_fabbook_secret';
	...
	public function auth_facebook(){
		try {
			...
			if ( isset($data['id']) && $user = $this->get_user($data['id']) ){
				...
				$userdata 	= ET_Seller::convert( $user );
				$nonce 		= array(
