...
// 		$makePOT	=	new MakePOT();
// 		$makePOT->xgettext('wp-theme', get_template_directory() , DEFAULT_LANGUAGE_PATH.'/'.$file_name);
...
// 		$mo->set_header( 'Project-Id-Version', 'Job Engine v'.CE_VERSION );
...
// 		$mo->set_header( 'Project-Id-Version', 'classifiedengine v'.CE_VERSION );
// 		$mo->set_header( 'Report-Msgid-Bugs-To', ET_URL );
// 		$mo->set_header( 'MO-Creation-Date', gmdate( 'Y-m-d H:i:s+00:00' ) );
// 		$mo->set_header( 'MIME-Version', '1.0' );
// 		$mo->set_header( 'Content-Type', 'text/plain; charset=UTF-8' );
...
// 		$mo->export_to_file(THEME_LANGUAGE_PATH.'/'.$selected_lang.'.mo');
...
function et_load_text_domain () {
	...
	et_create_content_directory_ce();
	if( in_array($selected_lang, get_available_languages(THEME_LANGUAGE_PATH)) )
		load_textdomain(ET_DOMAIN,THEME_LANGUAGE_PATH."/$selected_lang.mo");
	else
		load_textdomain(ET_DOMAIN, DEFAULT_LANGUAGE_PATH."/$selected_lang.mo");
...
function et_save_language () {
	...
	$language_files	=	array ('mo' =>$machine , 'po' => $people);
...
function et_add_new_lang () {
	...
	if( $lang_name== "" || in_array($lang_name, $lang_arr)) {
		...
		if( et_generate_mo($lang_name , 'ClassifiedEngine')) {
			// update language options
			$general_opts	=	new CE_Options();
			//$general_opts->set_language($lang_name);
...
function et_change_language ( $lang ) {
	...
	if( !in_array($lang, et_get_language_list(THEME_LANGUAGE_PATH) )) {
		return 0;
	}
...
function et_create_content_directory_ce ( $path	=	array ('/lang')) {
	et_create_content_directory (array('/classifiedengine', '/classifiedengine/lang'));
	// copy list file .mo to jobengine/lang
	$custom_langs	=	get_available_languages(ET_LANGUAGE_PATH);
	$je_lang_dir 	= WP_CONTENT_DIR . '/et-content/classifiedengine/lang/';
